...
from shared.box_client import BOX_BASE_URL, http_post, http_put

if len(sys.argv) > 1:
    os.environ["BOX_BASE_URL"] = sys.argv[1].rstrip("/")
    # Re-bind after env override: factory reads BOX_BASE_URL at call time.
BASE_URL = os.environ.get("BOX_BASE_URL", BOX_BASE_URL).replace("localhost", "127.0.0.1")
os.environ["BOX_BASE_URL"] = BASE_URL


async def upsert(api_key: str, cabinet: str, drawer: str,
    ...
    r = await http_put("/box/upsert", payload, api_key)
    r.raise_for_status()
    return r.json()["joke_id"]


async def main() -> None:
    r = await http_post("/accounts", {"name": "seed-account"})
    ...
    for cab, drw, fil, cat, text, score, reaction, p in jokes:
        await upsert(api_key, cab, drw, fil, cat, text, score, reaction, p)

    cab, drw, fil, cat, text, score, reaction, p = violation_joke
    violation_id = await upsert(api_key, cab, drw, fil, cat, text, score, reaction, p)
